...
set GitSrc=%~d0\install\resource\%Git%
...
if not exist %~d0\install\tmp (
        mkdir %~d0\install\tmp
)

type nul>%~d0\install\tmp\udisk.tmp
...
for /l %%a in (1,1,%CountVar%) do (
        ...
        for /f "tokens=1-3" %%b in ('reg query "%KeyName%" 2^>nul') do (
                if /i "%%b"=="!Num!" (
                        ...
                        echo,VID: !VID! PID: !PID! SN: !SN!>>%~d0\install\tmp\udisk.tmp
                )
        )
)
...
REM 运行install.sh
if exist %~d0\install\shell\install.sh (
        start %GitBash% %~d0\install\shell\install.sh
) else (
        call :EchoError "%~d0\install\shell\install.sh 文件不存在"
)
